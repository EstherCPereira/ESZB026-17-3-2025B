...
void enviaDadosPelaSerial(){              // opcional, para debug
  ...
  Serial.print("; Valor volts: "); Serial.print(valorvolts);
  ...
  Serial.print(buffer[3],HEX); Serial.print(" "); // byte mais significativo do valor lido
  Serial.print(buffer[4],HEX); Serial.print(" "); // byte menos significativo do alarme
  Serial.print(buffer[5]); Serial.print(" "); // byte mais significativo do valor em volts
  Serial.println(buffer[6]);                  // byte menos significativo do valor em volts
}

void loop(){                       // Le o valor no potenciometro a cada 1 segundo
  valorPot = analogRead(PinoPotenciometro); // le dado com ADC de 10-bit
  valorvolts = valorPot*5.0/1023.0;
  buffer[0] = valorPot & 0xFF;     // byte menos significativo
  buffer[1] = valorPot >> 8;       // byte mais significativo
  buffer[5] = valorvolts;
  buffer[6] = (valorvolts - buffer[5])*100.0;
